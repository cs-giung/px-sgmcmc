...
from src.model import PXResNet20, permute_params_apply
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--px_friction', default=1.0, type=float,
        help='friction coefficient for expanded matrices (default: 1.0)')
    parser.add_argument(
        '--px_l2_regularizer', default=2.0, type=float,
        help='L2 regularization for expanded matrices (default: 2.0)')
    ...
    model = PXResNet20(num_classes=num_classes)

    init_position = model.init(
        jax.random.PRNGKey(args.seed), jnp.ones((1,) + input_shape))['params']
    init_position = {'PQ': None, 'V': init_position}

    perm_params, op = model.apply(
        {'params': init_position['V']}, jnp.ones((1,) + input_shape),
        get_perm=True)
    ...
    def energy_fn(param, batch):
        ...
        logits = forward_fn(
            permute_params_apply(param['PQ'], op, param['V']), batch['inputs'])
        target = batch['target']
        ...
        neg_log_prior = 0.5 * sum(
            ...
            for e in jax.tree_util.tree_leaves(param['V']))
        posterior_energy = neg_log_likelihood + neg_log_prior

        px_l2_regularizer = 0.5 * sum(
            jnp.sum(e**2) for e in jax.tree_util.tree_leaves(param['PQ']))

        aux = OrderedDict({
            'posterior_energy': posterior_energy,
            'neg_log_likelihood': neg_log_likelihood,
            'neg_log_prior': neg_log_prior,
            'px_l2_regularizer': px_l2_regularizer})

        return posterior_energy \
            + args.px_l2_regularizer * px_l2_regularizer, aux

    @partial(jax.pmap, axis_name='batch')
    def update_fn(state, batch, step_size, friction):
        """Updates state."""
    ...
    init_perm_params = jax.tree_util.tree_map_with_path(
        lambda path, perm: jnp.eye(perm.shape[0]), perm_params)
    init_perm_moment = jax.tree_util.tree_map(jnp.zeros_like, init_perm_params)

    state = sghmc.SGHMCState(
        step=0, rng_key=jax.random.PRNGKey(args.seed),
        position={**init_position, 'PQ': init_perm_params},
        momentum={**init_momentum, 'PQ': init_perm_moment})
    friction = jax.tree_util.tree_map(
        lambda e: jnp.full_like(e, args.friction), state.position)
    friction['PQ'] = jax.tree_util.tree_map(
        lambda e: jnp.full_like(e, args.px_friction), state.position['PQ'])

    state = jax.device_put_replicated(state, jax.local_devices())
    friction = jax.device_put_replicated(friction, jax.local_devices())
    ...
    for sample_idx in range(1, args.num_samples + 1):
        ...
        for update_idx in range(1, args.num_updates + 1):
            ...
            step_size = jax.device_put_replicated(
                ...
                jax.local_devices())
            aux, state = update_fn(state, batch, step_size, friction)
            ...
            if update_idx == 1 or update_idx % 1000 == 0:
                ...
                merged_position = permute_params_apply(
                    jax.tree_util.tree_map(
                        lambda e: e[0], state.position['PQ']), op,
                    jax.tree_util.tree_map(
                        lambda e: e[0], state.position['V']))
                merged_position = jax.device_put_replicated(
                    merged_position, jax.local_devices())

                summarized['norm'] = float(jnp.sqrt(sum(
                    jnp.sum(e**2) for e in jax.tree_util.tree_leaves(
                        jax.tree_util.tree_map(
                            lambda e: e[0], merged_position)))))

                logits = make_predictions(merged_position, trn_images)
                summarized['trn/err'] = compute_err(logits, trn_labels)
                summarized['trn/nll'] = compute_nll(logits, trn_labels)

                logits = make_predictions(merged_position, val_images)
        ...
        summarized['norm'] = float(jnp.sqrt(sum(
            jnp.sum(e**2) for e in jax.tree_util.tree_leaves(
                jax.tree_util.tree_map(lambda e: e[0], merged_position)))))
